...
static inline float aspectConverter(const std::string &value) {
    try {
        if (value.empty()) {
            ...
            if (int index = value.find(":"); index >= 0) {
                // 比例模式
                auto prev = std::stof(value.substr(0, index));
                auto next = std::stof(value.substr(index+1,value.length()));
                brls::Logger::debug("Aspect {} {}", prev, next);
                return prev / next;
            } else {
                // 纯数字
...
void MPVCore::init() {
    ...
    // TODO mpv config dir
    mpvSetOptionString(mpv, "config-dir", "/config/mpv");
    ...
    command_async("set", "audio-client-name", "nxplayer");
                    ...
                    case 4:
                        ...
                        if (((mpv_event_property *)event->data)->data) {
                            ...
                            if (video_progress != (int64_t)playback_time) {
                                ...
                                mpvCoreEvent.fire(MpvEventEnum::UPDATE_PROGRESS);
                                // 判断是否需要暂停播放
                                if (CLOSE_TIME > 0 && Utils::getUnixTime() > CLOSE_TIME) {
                                    CLOSE_TIME = 0;
                                    this->pause();
                                }
                            }
...
void MPVCore::disableDimming(bool disable) {
    ...
    brls::Application::getPlatform()->disableScreenDimming(disable, "Playing video", "nxplayer");
